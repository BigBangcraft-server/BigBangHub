...
- **Aliases de minigame devolvidos aos plugins de backend (`/hg`, `/bedwars`)**:
  - `AliasCommandRegistrationTest` / `StuckMatchFixRegressionTest`: proxy **sem** aliases (`snapshot.aliases().isEmpty()`). O `/hg` do BigBangHungerGames (`/hg setpos1`) e o `/bedwars`/`/bw` do MBedwars deixam de ser interceptados pelo proxy; entrada nas filas continua por `/queue join <game>`, bússola e NPC.
