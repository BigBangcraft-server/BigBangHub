...
final class VelocityCommands implements SimpleCommand {
    ...
    private void executeHub(Invocation invocation) {
        ...
        switch (sub) {
            case "version" -> invocation.source().sendPlainMessage("BigBangHub " + BigBangHubVelocityPlugin.VERSION + " (Velocity 4.1.1)");
            case "reload" -> {
    ...
    private void status(CommandSource source) {
        source.sendPlainMessage("=== BigBangHub " + BigBangHubVelocityPlugin.VERSION + " Status ===");
